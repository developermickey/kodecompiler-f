...
import { 
  ...
  CheckCircle, AlertCircle, Clock, Tag, Building, Code, 
  Terminal, ChevronDown, ChevronUp, ChevronRight, AlertTriangle, 
  Info, Check, PlayCircle, Copy, BarChart3, Zap, Maximize2,
  ...
  useEffect(() =>
{
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const acceptanceRate = data.total_submissions > 0
          ? ((data.accepted_submissions / data.total_submissions) * 100).toFixed(2)
          : "0.00";
  ...
  useEffect(() =>
    {
      ...
      const fetchSubmissions = async () =>
      {
        ...
        {
          const res = await fetch(
            `http://localhost:5000/api/problems/submissions/my?problem_id=${Number(problemId)}`,
            {
              method: "GET",
              credentials: "include",
            },
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      ...
      if (passed) {
        ...
        setOutput(`❌ Wrong Answer\n\n✗ Test Case ${Math.floor(Math.random() * problem.testCases.length) + 1} failed\n\nInput: ${problem.testCases[0]?.input}\nYour Output: ${Math.random() > 0.5 ? 'null' : 'undefined'}\nExpected: ${problem.testCases[0]?.expected}`);
      }
      setIsRunning(false);
    }, 2000);
  };
  ...
  // Language options
  const languageOptions = Object.keys(problem.starter_code || {}).map(lang => ({
    value: lang,
    ...
    <div className={`h-screen flex flex-col ${bgPrimary} overflow-hidden`}>
      ...
      <div className={`${bgSecondary} border-b ${borderColor} px-5 py-2.5 flex items-center justify-between`}>
        ...
        <div className="flex items-center gap-4">
          <div className="text-center">
            ...
            <div className={`font-bold text-sm ${textPrimary}`}>{problem.acceptanceRate}%</div>
          </div>
          <div className="text-center">
            <div className={`text-xs ${textTertiary} mb-0.5`}>Points</div>
            <div className={`font-bold text-sm ${textPrimary}`}>{problem.points}</div>
          </div>
        </div>
      ...
      <div ref={containerRef} className="flex flex-1 overflow-hidden">
                        ...
                        <div className="p-4">
                          <div className="space-y-3">
                            <div>
                              ...
                              <pre className={`text-xs font-mono p-3 rounded ${darkMode 
                      ...
                      {submissions.map((sub) => {
                        ...
                        return (
                          <div 
                            ...
                            onClick={()=>{setSelectedCode(sub.code); setSeletcedsubLanguage(sub.language)}}
                          >
                            <div className={`col-span-3 text-xs ${textSecondary}`}>
                              {sub.date}
                            </div>
                            ...
                            <div className="col-span-2">
                              <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-medium border ${statusColors.bg} ${statusColors.text} ${statusColors.border}`}>
                                ...
                                {sub.status === 'Wrong Answer'? 'Wrong': sub.status === 'Time Limit Exceeded'? (<span className="inline-flex items-center gap-1"><Clock className="w-3 h-3 text-amber-600" />TLE</span>): sub.status}                              </span>
                            </div>
                            <div className={`col-span-2 text-xs font-mono ${textPrimary}`}>
                              {sub.runtime}
                            </div>
                            <div className={`col-span-2 text-xs font-mono ${textPrimary}`}>
                              {sub.memory}
        ...
        <div
          ...
          className={`w-1 cursor-col-resize ${darkMode 
        ...
        >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  <button
                    ...
                    className={`cursor-pointer flex items-center gap-2 px-3 py-1.5 rounded ${darkMode 
                      ? 'bg-zinc-800 hover:bg-zinc-700' 
                      : 'bg-gray-100 hover:bg-gray-200'
                    } border ${borderColor} text-sm font-medium ${textPrimary} transition-colors`}
                  >
                      ...
                      <div className="p-1">
                        {languageOptions.map((option) => (
                          <button
                            ...
                            className={`w-full flex items-center justify-between px-3 py-2 text-sm rounded ${selectedLanguage === option.value
                              ? `${darkMode ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-600'}`
                              : `${textSecondary} ${darkMode ? 'hover:bg-zinc-800' : 'hover:bg-gray-100'}`
                            } transition-colors`}
                          >
          ...
          >
            ...
            <div
              ...
              className={`h-1.5 cursor-row-resize ${darkMode 
                ? 'bg-zinc-800 hover:bg-blue-500' 
                : 'bg-gray-300 hover:bg-blue-400'
              } transition-colors`}
            />
